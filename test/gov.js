...
contract("Gov", accounts => {
    ...
    beforeEach(async () => {
        admin = accounts[1]
        bpro = await BPRO.new(admin, {from: admin})
        const bravoImpl = await BravoImpl.new({from: admin, gas:5e6})

        const nonce = await web3.eth.getTransactionCount(admin);
        const bravoAddress = web3.utils.toChecksumAddress("0x" + web3.utils.sha3(RLP.encode([admin,nonce+1])).slice(12).substring(14));
        timelock = await Timelock.new(bravoAddress, timelockDelay, {from: admin, gas:5e6})

        bravo = await Bravo.new(timelock.address, bpro.address, timelock.address, bravoImpl.address, 3 * blocksPerDay, 2 * blocksPerDay, web3.utils.toWei("2500"),{from: admin, gas:5e6})
        assert.equal(bravo.address, bravoAddress, "unexpected branvo address")
    ...
    it.skip("find cool address", async () => {
        let address = accounts[1]
        const nonce = 0
        let privateKey = web3.utils.sha3(RLP.encode([address,nonce]))
        console.log(privateKey)
        let bestPrefix = "0xb"

        //console.log(web3.eth.accounts.privateKeyToAccount("0x92b59e684d375700f0bf9f15f1a7d1a7c1b8a1e7699f95dc190c23d8c983ea9c"))
        //sd
        while(true) {
            address = web3.eth.accounts.privateKeyToAccount(privateKey).address
            const nextKey = web3.utils.sha3(RLP.encode([address,nonce]))
            const contractAddress = "0x" + nextKey.slice(12).substring(14)

            //console.log(nextKey)

            if(contractAddress.toLowerCase().startsWith(bestPrefix)) {
                console.log(privateKey, contractAddress)
                bestPrefix += "b"
            }
            //const address = web3.utils.toChecksumAddress("0x" + nextKey).slice(12).substring(14));
